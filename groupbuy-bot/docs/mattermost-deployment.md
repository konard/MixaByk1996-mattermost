...
```bash
docker compose -f docker-compose.with-mattermost.yml up -d mattermost-postgres mattermost
```
...
watch docker compose -f docker-compose.with-mattermost.yml ps mattermost
```
...
```bash
docker compose -f docker-compose.with-mattermost.yml up -d
```

### 6.3 Проверка статуса контейнеров

```bash
docker compose -f docker-compose.with-mattermost.yml ps
```
...
docker compose -f docker-compose.with-mattermost.yml logs -f mattermost-adapter

# Логи бот-сервиса (обрабатывает команды)
docker compose -f docker-compose.with-mattermost.yml logs -f bot
```

### 7.5 Проверка связи адаптера с Mattermost

```bash
docker compose -f docker-compose.with-mattermost.yml exec mattermost-adapter \
...
```bash
docker compose -f docker-compose.with-mattermost.yml restart mattermost
```
...
```bash
# База данных GroupBuy
docker compose -f docker-compose.with-mattermost.yml exec -T postgres \
  pg_dump -U postgres groupbuy | gzip > backup_groupbuy_$(date +%Y%m%d).sql.gz

# База данных Mattermost
docker compose -f docker-compose.with-mattermost.yml exec -T mattermost-postgres \
...
docker run --rm \
  -v groupbuy-bot_mattermost_data:/source:ro \
  -v $(pwd)/backups:/dest \
...
```bash
# crontab -e
0 3 * * * cd /opt/mattermost/groupbuy-bot && \
  docker compose -f docker-compose.with-mattermost.yml exec -T postgres \
...
```bash
docker compose -f docker-compose.with-mattermost.yml exec mattermost \
...
2. Измените тег образа в `docker-compose.with-mattermost.yml`:
   ```yaml
...
3. Потяните новый образ и перезапустите:
   ```bash
   docker compose -f docker-compose.with-mattermost.yml pull mattermost
   docker compose -f docker-compose.with-mattermost.yml up -d mattermost
   ```

4. Проверьте логи на наличие ошибок миграций:
   ```bash
   docker compose -f docker-compose.with-mattermost.yml logs mattermost | tail -50
   ```
...
docker compose -f docker-compose.with-mattermost.yml ps mattermost-postgres
docker compose -f docker-compose.with-mattermost.yml logs mattermost-postgres
...
docker inspect $(docker compose -f docker-compose.with-mattermost.yml \
...
docker compose -f docker-compose.with-mattermost.yml logs -f mattermost-adapter bot
```
...
```bash
docker run --rm \
  -v groupbuy-bot_mattermost_data:/data \
  -v groupbuy-bot_mattermost_config:/config \
  -v groupbuy-bot_mattermost_logs:/logs \
...
```bash
docker compose -f docker-compose.with-mattermost.yml stop mattermost mattermost-postgres mattermost-adapter
docker compose -f docker-compose.with-mattermost.yml rm -f mattermost mattermost-postgres

docker volume rm \
  groupbuy-bot_mattermost_data \
  groupbuy-bot_mattermost_config \
  groupbuy-bot_mattermost_logs \
  groupbuy-bot_mattermost_plugins \
  groupbuy-bot_mattermost_client_plugins \
  groupbuy-bot_mattermost_bleve_indexes \
  groupbuy-bot_mattermost_postgres_data
...
```bash
# Все сервисы сразу
docker compose -f docker-compose.with-mattermost.yml logs -f

# Конкретный сервис
docker compose -f docker-compose.with-mattermost.yml logs -f mattermost
docker compose -f docker-compose.with-mattermost.yml logs -f mattermost-adapter
docker compose -f docker-compose.with-mattermost.yml logs -f bot
```
